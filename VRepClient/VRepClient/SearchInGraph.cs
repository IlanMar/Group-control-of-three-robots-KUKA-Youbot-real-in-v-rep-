...
namespace VRepClient
{
    ...
    public class SearchInGraph
    {
        ...
        {//шаг 1
            for (int i = -1; i < 2; i++)//какаято херня чтобы точка осчета всегда была доступна
            {
                for (int k = -1; k < 2; k++)
                {
                    field[start.X + i, start.Y + k, 0] = 1;
                    field[start.X + i, start.Y + k, 1] = 0;
                }
            }
                ...
                for (int i = -3; i < 4; i++)
                {
                    for (int k = -3; k < 4; k++)
                    {
                        if (point.X + i > 0 && point.X + i < field.GetLength(0))
                        {
                            ...
                            {
                                if (field[point.X + i, point.Y + k,0] == 1)
                                {
                                    freeNode++;
                                }
                            }
